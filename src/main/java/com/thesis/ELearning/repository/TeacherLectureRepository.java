...
public interface TeacherLectureRepository extends JpaRepository<TeacherLectures, String> {
    @Query(value = "SELECT t from TeacherLectures t where (t.code like %?1%) or t.resource.teacher.user.email =?2 order by t.createdAt desc ")
    Page<TeacherLectures> getTeacherLecture(String search, String email , Pageable pageable);

    @Query(value = "SELECT t from TeacherLectures  t where t.code = ?1 and t.resource.teacher.user.email = ?2")
